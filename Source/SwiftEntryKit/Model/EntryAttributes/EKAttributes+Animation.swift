...
public extension EKAttributes {
    ...
    struct Animation: Equatable {
        ...
        public var containsAnimation: Bool {
            containsTranslation || containsScale || containsFade
        }

        /** Returns the maximum delay amongst all animations */
        public var maxDelay: TimeInterval {
            max(translate?.delay ?? 0, max(scale?.delay ?? 0, fade?.delay ?? 0))
        }

        /** Returns the maximum duration amongst all animations */
        public var maxDuration: TimeInterval {
            max(translate?.duration ?? 0, max(scale?.duration ?? 0, fade?.duration ?? 0))
        }
