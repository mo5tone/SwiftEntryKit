...
extension EKContentView {
    ...
    private func shouldStretch(with translation: CGFloat) -> Bool {
        if attributes.position.isTop {
            translation > 0 && inConstraint.constant >= inOffset
        } else {
            translation < 0 && inConstraint.constant <= inOffset
        }
    }
    ...
    private func animateRubberBandPullback() {
        ...
        let animation: EKAttributes.Scroll.PullbackAnimation = if case .enabled(swipeable: _, pullbackAnimation: let pullbackAnimation) = attributes.scroll {
            pullbackAnimation
        } else {
            .easeOut
        }
    ...
    private func testSwipeInConstraint() -> Bool {
        if attributes.position.isTop {
            inConstraint.constant < inOffset
        } else {
            inConstraint.constant > inOffset
        }
    }
